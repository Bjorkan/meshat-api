...
Meshat.se MCP-V2 is the public, anonymous, read-only MCP interface for Meshat.se. It serves stateless Streamable HTTP only at `POST /mcp` and exposes process health at `GET /healthz` (including the configured `MCP_RELEASE_ID`, default `2.0.0`) and REST-backed readiness at `GET /readyz`, which reports this service's `release_id` plus the REST release id, schema version, and schema fingerprint so REST/MCP deployment drift is observable from a fresh session. Other `/mcp` methods receive a JSON-RPC-compatible `405` with `Allow: POST`, and malformed JSON receives a sanitized JSON-RPC parse error.
