# Meshat.se REST API

Production REST domain API for Meshat.se. Access is public, anonymous, read-only, and rooted at `/v1`. PostgreSQL is accessed only by this service as `meshcore_http`, through fixed parameterized queries against `meshcore_public`.

## Public Surface

- `/`, `/healthz`, `/readyz`: service metadata, liveness, and database readiness.
- `/docs`: public Swagger UI.
- `/openapi.json`: OpenAPI document without authentication schemes.
- `/v1/docs`: cached Meshat.se documentation content. This is distinct from Swagger.
- `/v1/sources`, `/v1/meshcore`: source and resource discovery.
- `/v1/meshcore/nodes`: nodes, detail, aggregated neighbors, adverts, sightings, and telemetry.
- `/v1/meshcore/observers`: observers, detail, current status, and metrics.
- `/v1/meshcore/iata`: configured geographic MQTT ingress areas.
- `/v1/meshcore/regions`: logical MeshCore regions derived from neighbor scopes.
- `/v1/meshcore/packets`: packets, raw MeshCore bytes, and public observations.
- `/v1/meshcore/messages`, `/telemetry`, `/traces`: public protocol projections.
- `/v1/meshcore/stats`, `/activity`: current summary and bounded time series.

There are no table, schema, arbitrary query, SQL, write, login, or authentication routes.

## Domain Semantics

IATA and MeshCore regions are intentionally separate. IATA codes such as `JKG` identify geographic MQTT ingress/hearing areas stored in public `iata` columns. Public `regions` are logical neighbor scopes derived only from neighbor `scope` membership and served from the broker-maintained `region_scopes` registry: built-in Swedish codes (`se`, `seXX` county, `seXXXX` municipality with hardcoded official municipality names such as `Halmstads kommun`) plus every scope detected in neighbor evidence. Region responses expose `region` (canonical lowercase scope), `name`, `first_seen`, `last_seen`, `manually_added`, `observation_count`, node/observer counts, `last_activity`, and `links` for nodes and observers (there is no activity link because per-observation region attribution does not exist). `/v1/meshcore/regions` is a bounded keyset-paginated catalog with `observed_only`, `manually_added`, and `prefix` filters; registered regions with no evidence yet return zero counts. Swedish region inputs — including the `prefix` filter — are case-insensitively normalized to lowercase; unknown custom scopes keep their original case.

Region membership is always derived from one canonical entity-region evidence relation: a node or observer matches `region=` only when its own public key carries retained scope evidence, never because it reported neighbors in that region, and `observer_count` in a region summary equals the distinct observers matching `search_observers(region=...)`. When a region/IATA filter is combined with `seen_from`/`seen_to`, the same evidence population must fall inside the time window (sighting receipt time for IATA, snapshot evidence time for regions), not just the node's last-seen timestamp. `/v1/meshcore/activity` accepts only `window`, `interval`, and geographic `iata` — there is no `region` filter until per-observation region attribution evidence exists in the data model.

Messages are deduplicated by stable logical packet identity and expose canonical fields (`iata`, `observation_count`, `representative_packet_sha256`, `first_received_at`, `last_received_at`) that are identical between search and detail for the same `lp_`-prefixed ID; search results additionally carry `matched.iata` and `matched.observation_count` describing only the query-scope evidence. Packet search accepts `logical_id` to list every physical packet variant of one logical message. Traces are observation-level events with `observer` and `logical_id` identity fields. Stats expose `regions` as `{ configured, observed }` populations and all aggregate counts as JSON numbers.

Observer locations use the verified location of the node with the same public key. Radius searches use the public PostGIS geography index while responses use normalized latitude/longitude columns. Neighbor responses aggregate only each reporting observer's latest snapshot. `reciprocal` means both the requested node reports the counterpart and the counterpart reports the requested node; one-sided evidence remains `reported` and includes its direction.

Packet `raw` is the MeshCore packet byte sequence encoded as lowercase `0x` hex. Packet observation paths and trace hops preserve ordered resolved, ambiguous, and unresolved information: a stored ingestion-time resolution means `resolved`, more than one current trusted prefix candidate means `ambiguous` (candidate state, not guaranteed ingestion-time state), and otherwise the hop is `unresolved`. Late candidates never retroactively resolve historical hops. Node, advert, and neighbor roles are normalized to lowercase; role filters are case-insensitive. Private MQTT receipts and metadata are never queried or returned. Messages are deduplicated by stable logical packet identity and aggregate their matching IATA observations; a message ID always uses the required `lp_` prefix, and `representative_packet_sha256` names the packet behind the latest matching observation. Packet search accepts `logical_id` to list every physical packet variant of one logical message. Message defaults and maxima are configured by `MESSAGE_DEFAULT_LIMIT` and `MESSAGE_MAX_LIMIT` (50 and 200 by default). Observer `active` means accepted ingest within `OBSERVER_ACTIVE_WINDOW_MS`, five minutes by default; it does not claim a live MQTT socket. The configured Swedish primary/secondary IATA mapping is REST-owned domain data copied from the reference broker configuration; no external attribution source was identified beyond that configuration.

Large collections use versioned opaque keyset cursors. Cursors contain a query fingerprint and cannot be reused with different filters, sort, or order. They require no server-side session state.

## Documentation Cache

At startup the service shallow-clones `DOCS_GIT_REPOSITORY` into an isolated temporary checkout, validates its Git metadata and non-symlinked docs root, then swaps it into `DOCS_CACHE_DIR`. Repository URLs containing credentials are rejected. Stale metadata is read from the cached checkout's actual origin, checked-out ref, and commit; a cache from a different configured repository/ref is not served.

Only lowercase `**/*.md` files and exactly `meshtastic/example.yaml` beneath `DOCS_SUBDIR` are public. Other existing assets return `NOT_FOUND`. Public documents must be valid UTF-8 and are always returned with `encoding: "utf-8"`; binary/base64 responses are never produced. `DOCS_MAX_FILE_BYTES` defaults to and cannot exceed 65536 bytes. Oversized or malformed UTF-8 documents are excluded from index and search, while direct oversized retrieval returns `413`.

Traversal, absolute paths, `.git`, and symlink escapes remain rejected. Search considers public candidates in deterministic path order and scans at most 100 files and 4 MiB per request. Its non-cursor result contains `query`, `limit`, `returned`, `total_matches`, `scan_complete`, `truncated`, and `results`; `total_matches` covers scanned candidates, and `scan_complete` states whether that is the complete candidate set. A failed refresh leaves a matching prior tree untouched and serves it as `stale`; without a valid matching checkout, docs return `DOCS_UNAVAILABLE` while the core API remains running.

Startup refresh completes before the HTTP listener opens, and the supported Compose deployment runs one REST replica. The cache swap is designed for that lifecycle; cross-process writers to the same docs volume are not supported.

The runtime image includes Git and runs as the non-root `node` user. The deployment must make the configured cache directory writable, normally with the root Compose named volume.

## Configuration

Configuration comes from environment variables, normally the root `.env`. Important values are `DATABASE_HOST`, `DATABASE_PORT`, `DATABASE_NAME`, `DATABASE_USER=meshcore_http`, `DATABASE_PASSWORD`, `DATABASE_SSL`, `DATABASE_POOL_MAX`, `OBSERVER_ACTIVE_WINDOW_MS`, `REST_HOST`, `REST_PORT`, `TRUST_PROXY`, CORS/rate-limit settings, and the `DOCS_*` settings. Invalid values fail startup. No password-file or authentication settings are supported.

`TRUST_PROXY` defaults to false, preventing untrusted forwarding headers from controlling IP rate limits; the rate-limit allowlist exempts only `/healthz` and `/readyz` — the HTTP `Host` header is client-controlled and never grants a bypass. The database pool defaults to four connections and production closes it during graceful shutdown. Readiness requires public schema ID `meshcore-mqtt-broker-postgres-v1` **and** a stored SHA-256 fingerprint of the public database contract matching a live recomputation from the catalog; any mismatch returns 503. The canonical schema version is `12`. Readiness deliberately accepts the bridge versions `9`, `10`, `11`, and `12` while production databases migrate (`src/repository.ts`): version `9` is validated with its legacy fingerprint format, which includes ordinary performance indexes, while versions `10` through `12` use fingerprint-v2, which excludes them. An unsupported version also returns 503. `RELEASE_ID` (default `1.0.0`) is published in the root metadata, `/readyz`, and the OpenAPI version so REST/MCP deployment drift is observable.

## Development

Code quality (Bun 1.4.0 toolchain): `bun run lint` (ESLint, type-aware, zero warnings tolerated), `bun run format` / `bun run format:check` (Prettier), and `bun run check` (format + lint + typecheck + test). SQL runs on Bun.SQL directly behind the database/repository layer: queries live in `src/repository.ts` as tagged templates so every runtime value is bound as a parameter, conditional predicates compose from other tagged templates, sort columns come from static per-endpoint allowlist maps, and sort direction is one of two static fragments. `.unsafe()` is prohibited in production source (`src/**`). When query semantics change, run the real PostgreSQL integration suite.

Public API contracts are Zod 4 single-source: `src/contracts.ts` defines every response contract (with stable registry IDs for reusable OpenAPI components) and `src/request-schemas.ts` defines every querystring/param contract with canonicalizing transforms, strict unknown-parameter rejection, cross-field refinements and bounded limits. The official `@fastify/type-provider-zod` compiles request validation, response serialization, TypeScript types, and — through `jsonSchemaTransform` + `jsonSchemaTransformObject` — the OpenAPI 3.1 document from these same schemas. Serialization whitelists undeclared response fields; a response that violates its own contract surfaces as a generic internal error while the root cause is logged.

```sh
bun install --frozen-lockfile
bun test
bun run typecheck
```

Run the workspace deployment from the root with `docker compose up -d`. Use `bun run test:live` only against an explicitly selected `API_BASE_URL`; it performs anonymous read-only requests.
