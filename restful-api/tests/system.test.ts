...
describe("public domain API", () => {
  ...
  it("exposes health, source discovery, and MeshCore overview anonymously", async () => {
    ...
    const source = (await app.inject("/v1/sources")).json().data[0];
    expect(source.url).toBe("/v1/meshcore");
    expect(
      (await app.inject("/v1/meshcore")).json().data.resources.regions,
    ).toBe("/v1/meshcore/regions");
    expect(
      (await app.inject("/v1/sources")).json().data[0].data_availability
        .telemetry.status,
    ).toBe("limited");
  });
  ...
  it("validates node filters and binds cursors to the normalized query", async () => {
    ...
    expect(mismatch.json().error.code).toBe("INVALID_CURSOR");
  });

  it("requires complete geographic filters and allowlisted sorts", async () => {
    expect(
      (await app.inject("/v1/meshcore/nodes?near_lat=57")).statusCode,
    ).toBe(422);
    expect(
      (await app.inject("/v1/meshcore/nodes?sort=private_id")).statusCode,
    ).toBe(400);
    expect(
      (
        await app.inject(
          "/v1/meshcore/observers?near_lat=57&near_lon=14&radius_km=20",
        )
      ).statusCode,
    ).toBe(200);
    expect(
      (await app.inject("/v1/meshcore/nodes?unsupported=value")).statusCode,
    ).toBe(400);
  });
  ...
  it("keeps IATA and logical regions separate", async () => {
    ...
    expect(iata.json().data).toMatchObject({
      code: "ARN",
      type: "secondary",
      primary_code: "STO",
    });
    expect(repository.lastIataCode).toBe("STO");
    expect(iata.json().data.links.nodes).toContain("iata=STO");
    const regions = await app.inject("/v1/meshcore/regions");
    expect(regions.json().data).toEqual([
      {
        region: "public",
        name: "public",
        first_seen: null,
    ...
    expect(se01.json().data).toMatchObject({
      region: "se01",
      name: "Stockholms län",
    });
  });
  ...
  it("exposes every contracted detail/history route", async () => {
    ...
    for (const route of routes)
      expect((await app.inject(route)).statusCode, route).toBe(200);
  });

  it("returns explicit non-cursor documentation search metadata", async () => {
    const search = (await app.inject("/v1/docs/search?q=mesh&limit=7")).json();
  ...
  it("returns stable not-found and readiness errors with request IDs", async () => {
    ...
    expect(readiness.json().error.code).toBe("DATABASE_UNAVAILABLE");
  });
  ...
  it("rejects unknown IATA and inverted time ranges consistently", async () => {
    ...
    expect(unknown.json().error.code).toBe("INVALID_ARGUMENT");
    const missing = await app.inject("/v1/meshcore/iata/ZZZ");
    expect(missing.statusCode).toBe(404);
    expect(missing.json().error.code).toBe("NOT_FOUND");
    const bareMessageId = await app.inject(
      `/v1/meshcore/messages/${"a".repeat(64)}`,
    );
    expect(bareMessageId.statusCode).toBe(400);
    expect(bareMessageId.json().error.code).toBe("INVALID_ARGUMENT");
    const bareLogicalId = await app.inject(
      `/v1/meshcore/packets?logical_id=${"a".repeat(64)}`,
    );
    expect(bareLogicalId.statusCode).toBe(400);
    expect(bareLogicalId.json().error.code).toBe("INVALID_ARGUMENT");
  ...
  it("publishes complete anonymous OpenAPI and Swagger UI", async () => {
    ...
    expect(
      document.paths["/v1/meshcore/observers"].get.parameters.some(
        (parameter: { name: string }) => parameter.name === "role",
      ),
    ).toBe(false);
    expect(
      document.paths["/v1/docs/{*}"].get.parameters.find(
        (parameter: { name: string }) => parameter.name === "*",
      ).required,
    ).toBe(true);
    expect(
      document.paths["/v1/meshcore/packets"].get.responses["503"],
    ).toBeTruthy();
    ...
    expect(
      document.paths["/readyz"].get.responses["200"].content["application/json"]
        .schema.properties.data.required,
    ).toEqual(["status", "database", "docs"]);
    for (const [path, pathItem] of Object.entries(document.paths) as Array<
      [string, { get?: Record<string, any> }]
    >) {
      ...
      expect(
        success?.content?.["application/json"]?.schema?.example,
        path,
      ).toBeTruthy();
...
describe("configured message bounds", () => {
  it("uses environment-derived message defaults and maxima", async () => {
    ...
    expect(
      (await app.inject("/v1/meshcore/messages?limit=10")).statusCode,
    ).toBe(400);
