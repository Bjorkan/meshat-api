 ...
 *
 * Canonical form is lowercase: `se` (Sweden), `seXX` (county), `seXXXX`
 * (municipality). These are MeshCore neighbor scopes/regions and are
 * intentionally distinct from the three-letter geographic IATA MQTT ingress
 * codes in `iata.ts`.
 *
 * Source: Statistiska centralbyrån (SCB), "Län och kommuner 2026", mirrored
 * from the canonical broker registry in meshcore-mqtt-broker/src/region-scopes.ts.
 */
...
/** Returns the canonical lowercase scope for a Swedish code, or the trimmed input unchanged. */
...
/** Returns the Swedish administrative name for a canonical scope, or `null`. */
export function regionScopeName(value: string): string | null {
  const normalized = normalizeRegionScope(value);
  if (normalized === "se") return "Sverige";
