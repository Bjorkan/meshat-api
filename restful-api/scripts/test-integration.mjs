...
export function runIntegration(runCommand = run) {
  let exitCode = 1;
  try {
    if (runCommand("bun", [path.join(brokerRepo, "scripts/test-db-up.mjs")]) !== 0) return 1;
    if (
      runCommand("bun", [path.join(here, "integration-provision.mjs")], {
        MESHCORE_BROKER_REPO: brokerRepo,
      }) !== 0
    )
      return 1;

    exitCode = runCommand("bun", ["test", "tests/integration"], {
    ...
    const cleanupCode = runCommand("bun", [path.join(brokerRepo, "scripts/test-db-down.mjs")]);
