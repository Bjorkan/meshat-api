...
## 2026-09-06 17:58 CEST — Muse Spark (muse-spark) via OpenCode

- Commit: accompanying `feat(rest,mcp): add message plaintext text filter` commit.
- `GET /v1/meshcore/messages` and MCP `search_messages` now accept an optional `text` filter: case-insensitive literal plaintext substring match (`ILIKE %...%` with `%`, `_`, `\` escaped, so wildcards/injection input match literally). Max 200 chars, trimmed, NUL/blank rejected as 400 like other free-text filters; unknown `query`/`contains` params stay 400 (strictObject).
- Wiring: `MessageFilters.text` (domain), `messageQuery` schema + `server.ts` mapping, repository applies the predicate in both the raw qualifier and matched-summary phases (new `message.text AS message_text` base column), cursor fingerprint binds it automatically; MCP `search_messages` exposes `text` (max 200, mirrors REST) and passes it through to REST.
- Encrypted/NULL-text messages never match a text filter (`ILIKE NULL` is not true); matched/canonical semantics unchanged (matched counts only text-matching observations, totals stay full, global representative kept).
- Added REST + MCP regression tests (forwarding/trimming/bounds, NUL case, MCP passthrough + overlong rejection). Verification: REST `bun run check` (71 pass, 45 integration-skipped), MCP `bun run check` (24 pass), REST `test:integration` against disposable PostgreSQL (45 pass), plus live probes: `jesper`/`Jesper` case-insensitive hit with matched=1/total=2, literal `%`/`_` handling, injection literal returns 0 rows, HTTP cursor page 1→2 walk under `text=` filter, 422 on cursor reuse with different text. No broker or production changes; no new index (broker-owned per TODO policy).

## 2026-09-06 (Europe/Stockholm) — Muse Spark (muse-spark) via OpenCode — Phase 1: full MCP coverage

- Commit: accompanying `feat(mcp): cover all REST domain routes, drop get_source` commit.
- MCP grew 23 → 32 tools so every REST domain route is reachable: list_node_adverts, list_node_sightings, list_node_telemetry, get_observer_status, list_observer_metrics, list_region_nodes, list_packet_observations, get_telemetry, get_trace, get_trace_hops. Removed duplicate get_source (identical endpoint to get_meshcore_overview, phantom ignored arg).
- Polished list/detail descriptions: pagination looping, text-plaintext-only note, channel vs channel_name, telemetry encrypted-payload limits, stats 24h/window definitions, active-window length, prefix normalization, link follow-ups.
- Verification: MCP `bun run check` 25 pass; REST `bun run check` 71 pass (45 integration skipped); README tool table, manifest count (32), live-manifest count updated.

## 2026-09-06 (Europe/Stockholm) — Muse Spark (muse-spark) via OpenCode — Phase 2: REST depth + MCP lockstep

- Commit: accompanying `feat(rest,mcp): expose RF evidence and history depth` commit.
- REST: packet observations gain suspected_mqtt_duplicate/suspected_rf_retransmission, hop_count, and path-hop candidates (node_prefix_candidates join, same pattern as trace hops; fixed a candidate-column SQL bug caught by live probe: 503 DATABASE_UNAVAILABLE surfaced the bad inner alias before any test did). Message/Telemetry/Trace/Sighting gain packet_observation_id linkage to the exact RF observation. Node gains latest_advert_at. New GET /observers/:pk/status-history and GET /observers/:pk/neighbor-snapshots (self-scopes + report completeness), with cursor resources registered.
- MCP lockstep: strict output schemas extended in the same change (node latest_advert_at, observation/truthy link ids, dup flags, hop candidates) plus list_observer_status_history and list_neighbor_snapshots tools (34 tools total).
- Verification: REST check 72 pass, MCP check 25 pass, test:integration 45 pass (real PostgreSQL), plus live probes against disposable DB: dup flags true/false per fixture, hop candidates [] for candidate-less prefix, text-filter hit with matched=2/total=2 and observation link. No broker changes (all columns already in meshcore_public).

## 2026-09-06 (Europe/Stockholm) — Muse Spark (muse-spark) via OpenCode — Phase 3: contract docs + hygiene

- Commit: accompanying `docs(contract): align docs with implemented API` commit.
- API-CONTRACT.md: message filter list gains `text` (+plaintext-only note); stats shape corrected (regions object, active_window_seconds, per-metric definitions replacing "document each metric"); activity `region` corrected to intentional omission; phantom INVALID_PUBLIC_KEY/INVALID_IATA codes removed with actual INVALID_ARGUMENT/NOT_FOUND mapping documented; MCP table 23 → 34 tools with all new routes; observers gain status-history/neighbor-snapshots sections; docs-search/file envelopes, packets logical_id, region query params documented.
- PROMPT.md (explicitly approved): activity region note corrected, message filter list gains text, MCP tool list updated (get_source removed, new tools added). mcp-server-v2/AGENTS.md tool list updated. Root README: REST observer row gains new routes, MCP 23 → 34 tools, message text-search example added.
- Verification: REST check 72 pass, MCP check 25 pass. No code changes in this phase.

## 2026-09-06 (Europe/Stockholm) — Muse Spark (muse-spark) via OpenCode — Neighbor-klassning: path-evidence, 150 km-gräns, positionsflytt rensar evidence

- Broker (`Bjorkan/meshcore-mqtt-broker`, ej pushad ännu): ny `dropStaleNeighborEvidenceOnRelocation` i `mqtt-history.ts` — vid verifierad advert med ny position (annan lat/lon än nodens lagrade) raderas nodens egna snapshots samt entries i andras snapshots som listar noden; scopes följer via CASCADE och berörda regionaggregat byggs om via ny `regionScopesForSnapshots` i `region-scope-aggregate.ts`. Adverts/sightings/paths lämnas orörda. Tester: nytt relocation-test + nytt 3-byte-klassningstest i `mqtt-history.test.mjs`; full `test:postgres` 303 pass. Not: `nodePositionBefore` läses före ingest (latest_* kan peka på äldre advert vid samma-ms-race), UNION-parametrar behövde `::text`-casts.
- REST (`Bjorkan/meshat-api`, ej pushad ännu): `getNeighborEvidence` bygger nu `resolved_path_pairs` (intilliggande hopp, båda 3 byte + resolved; packet-hopp via lagrad status, trace-hopp via exakt-en-kandidat-regeln) och exponerar `path_last_heard` i `evidence`; `aggregateNeighbors` filtrerar par utan känd position hos båda eller >150 km (`ST_Distance <= 150000`), rapport/path är ELLER-villkor. Kontrakt: `evidence.path_last_heard` tillagd; MCP-scheman + beskrivningar uppdaterade i lockstep.
- Verifiering REST: `bun run check` 73 pass (rest) + 25 pass (mcp); `test:integration` 47 pass mot riktig PostgreSQL (ny (S)-svit: 150 km-behåll/släpp, path-evidence). SQL: endast Bun.SQL-taggade parametrar, inga nya index.

## 2026-09-06 18:54 CEST — Muse Spark (muse-spark) via OpenCode — GHCR publish (:latest) efter gröna tester

- Commit: accompanying `ci: publish restful + mcp images to GHCR on main` commit.
- Nytt `publish`-jobb i `.github/workflows/ci.yml` (enda ändringen, +46 rader): bygger och pushar `ghcr.io/bjorkan/meshat-rest-api:latest` + `:sha-<12>` och `ghcr.io/bjorkan/meshat-mcp-v2:latest` + `:sha-<12>` (linux/amd64+arm64) vid varje push till main. Samma pinade actions som resten av filen (checkout v7.0.1, build-push v6.9.0, QEMU/setup-buildx v4); login endast mot GHCR med `GITHUB_TOKEN`, `packages: write` enbart på publish-jobbet.
- Gating exakt enligt krav: `needs: [rest, mcp, rest-integration, docker-build]` — fallerar restful ELLER mcp (eller integration/docker-build) så körs publish aldrig och inga packages skapas. `if` begränsar dessutom till push-event på `refs/heads/main` i `Bjorkan/meshat-api` (inga PR:er, inga forks).
- Verifiering denna session: `bun run check` grön i båda projekten (rest 73 pass/48 skip/0 fail; mcp 25 pass/0 fail), workflow-YAML parsad med PyYAML (5 jobb, needs/if/permissions/7 steg korrekta), båda Dockerfilerna byggda lokalt med `docker build` utan fel.

## 2026-09-06 (Europe/Stockholm) — Muse Spark (muse-spark) via OpenCode — Neighbors-fråga: 3,9 s → 275 ms + namnbugg-fix

- Commit: accompanying `perf(rest): anchor neighbor path pairs on the queried node` commit.
- `getNeighborEvidence` beräknade globala path-par (resolved_path_pairs/path_pairs över ~2M packet_path_hops + global self-join) per anrop → 3,8 s i produktion. Ny form: förankrad på den frågade noden (`resolved_node_public_key = key`, partnern via UNIK-index (path_id, hop_index), hopp_index ± 1, packet + trace), max-aggregering per motpart. Ekvivalens verifierad mot produktion för 10 noder: endast avsedda skillnader (se namnfix nedan); path_last_heard/within_range/direction/snr/regions identiska.
- Två medföljande korrigeringar: (1) namnbugg — frågan valde `node.latest_name` från den frågade nodens egen rad så varje neighbor visade nodens eget namn; nu `counterpart.latest_name/latest_role`, integrationstest (S) assertar motpartens namn. (2) trace-par använder `resolved_node IS NOT NULL` (brokerns resolvePrefix-semantik: exakt en betrodd kandidat) i stället för den för strikta `count(candidates)=1`-kontrollen.
- Verifiering: `bun run check` 73 pass; `test:integration` 47 pass mot riktig PostgreSQL; produktions-EXPLAIN 3858 → 275 ms; live `/neighbors` 200. Deployment: GHCR `:latest`-images för restful/mcp/broker (docker-compose image-lines, inga lokala builds), meshdb `/dev/shm` 128 MB → 1 GB (parallell DSM kraschade annars: "no space left on device" → 503), MCP_BUILD_SHA=ef1913653a6d, rollback-taggar + compose-backuper tagna före byte.

- Deployment-addendum (samma session): efter CI-publicering av 56493f3 drogs nya `:latest`-images och stacken återskapades på auth.se. Verifierat live: `/neighbors` 0,22–0,43 s (tidigare 2,5 s/503), motpartsnamn korrekta per rad (t.ex. SE1380-StrobeLight-4727 i stället för frågade nodens eget namn på alla rader), REST live smoke 22× 200, MCP live smoke PASSED (första försöket efter recreation misslyckades transient medan containern startade), alla fyra containrar healthy på `ghcr.io/bjorkan/...:latest`.

## 2026-09-06 (Europe/Stockholm) — Muse Spark (muse-spark) via OpenCode — Neighbors fulla ELLER-regel + 7-dagars TTL

- Commit: accompanying `feat(rest,mcp): path-only neighbor pairs and 7-day evidence freshness` commit.
- Full OR-semantik: par med endast path-evidence (ingen /neighbors-rapport) returneras nu som `relationship: "path"`, `direction: "path"`, `report_count: 0` — tidigare drevs resultatet enbart av rapportrader och rena path-par föll bort. `getNeighborEvidence` UNION-ar rapportrader med path-only-rader (NOT EXISTS mot evidence); mappern räknar rapportrader per rad (`isReport` baseras på radens direction, inte paret — bugg fixad där path-only-rader räknades som rapporter) och klassar par utan rapporter som "path".
- Ny TTL-regel: par kräver bevis inom 7 dagar (senaste av rapport-mottagning eller path-genomgång), annars tas kopplingen bort — query-time-filter i mappern (`NEIGHBOR_EVIDENCE_TTL_MS` = 604 800 000 ms), inte broker-radering (snapshothistorik och regionsaggregat får inte förstöras). Äldre rapportrader räknas fortfarande i report_count om paret hålls levande av färsk path.
- Verifiering: REST check 75 pass, MCP check 25 pass, test:integration 46 pass mot riktig PostgreSQL — (S)-sviten utökad med E (path-only inom 150 km → "path"), G (path-only utanför → droppas), H (path-only äldre än 7 dagar → droppas); enhetstester för path-only, mixed (gammal rapport + färsk path) och TTL-utgång. Kontrakt: relationship/direction-enum utökade med "path", report_count/observer_count min(0); API-CONTRACT §4, route- och MCP-beskrivningar uppdaterade. SQL-fix under vägen: dubblerad `)` i combined-CTE:n upptäckt via postgres-logg (503-mappning), verifierad med EXPLAIN-form på prod (148 ms för Fagerslätt-noden: 2 rapport-par + 1 path-par mot Öxnehaga).

- Deployment-addendum (samma session): CI-grönt efter kompletterande contracts.ts-commit (6470b80 — den glömdes i 20e5f73:s git-add och fångades av CI-typechecken, publish skippades som designat); nya `:latest`-images dragna och stacken återskapad på auth.se. Live-verifierat med användarens testnod SE-JKG-Fagerslätt (030680D1): 3 neighbors i stället för 2 — Öxnehaga-7316 syns nu som relationship/direction "path" med report_count 0 och path-evidence 2026-09-04 (inom TTL); HuskvarnaS/UGORptr kvarstår som reported/outbound med path-annotering. Timing 0,33–0,35 s per anrop. REST live smoke 22× 200, MCP live smoke PASSED (transient fail direkt efter recreation), båda containrarna healthy.
